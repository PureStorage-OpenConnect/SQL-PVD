...
Example script to create an environment that could host a 2-node SQL Server Failover Cluster Instance on Azure VMs.
This script steps through the creation of the VMs, but includes assumptions on pre-defined implementation points. As an example, it does not include the creation of the WSFC or SQL FCI, as all environments differ on setup and configuration of these features. It also connects the VM to the FlashArray or Cloud Block Store deployment.
